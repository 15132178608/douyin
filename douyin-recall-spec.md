# 历史归档：抖音收藏回忆工具最初工作清单与技术规格

> **历史文档，不代表当前实现。** 这是项目启动阶段的需求与技术决策快照；下方的勾选项、Schema、目录树和“不做”清单均未按后续实现逐项回填，请勿据此判断当前功能或代码结构。
>
> 此后项目已经加入喜欢内容线、自动分类、本地账号与用户隔离、后台任务、维护与备份等能力。当前事实请以 [README](README.md)、[当前架构](docs/architecture.md) 和 [当前路线图](docs/roadmap.md) 为准。
