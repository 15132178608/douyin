...
| **服务生命周期** | ✅ | `recall serve` 写 PID 状态、防重复启动；`recall status` 显示服务审计、端口 owner 和下一步建议；`recall stop` 只停止本项目记录的 Web 服务；Windows 开始菜单提供控制入口和账号恢复入口 |
...
长期使用时可以打开 `http://127.0.0.1:8000/maintenance`，查看服务、最近同步、索引、备份和后台队列状态，也可以手动执行一次标准维护、立即生成 SQLite 备份、只读校验最新备份、校验后恢复已有备份，或导出脱敏诊断包。

查看/停止本地 Web 服务；`status` 会显示服务审计、端口 owner PID 和下一步建议：
