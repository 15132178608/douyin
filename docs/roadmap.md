...
- ⏳ 首次启动仍偏工程化：会自动跑依赖准备，但还没有产品化进度页
- ⏳ 失败提示需要进一步收敛成普通用户能理解的中文提示
- ⏳ 安装包未签名，Windows SmartScreen 可能提示风险

### 首次使用流程
- ⏳ 还没有完整向导：欢迎页、环境检查、扫码登录、抓取选择、抓取进度、完成页
- ⏳ 登录态失效后的重新扫码流程需要更明确
...
### 高优：个人工具可安装、可启动
1. **安装包首启体验**——依赖准备、Playwright 安装、数据库初始化都有明确进度和失败重试
2. **首次启动向导**——欢迎页 → 环境检查 → 扫码登录 → 选择抓收藏/喜欢 → 抓取进度 → 完成
3. **doctor 命令扩展**——检查 web、jobs worker、SMTP、浏览器 profile、头像缓存目录、模型缓存

### 高优：个人数据安全
4. **备份 / 恢复 UI**——一键备份 SQLite，一键恢复，默认保留最近 N 份
5. **诊断包导出**——只导出脱敏日志和环境信息，不包含 `.env`、数据库、浏览器 profile
