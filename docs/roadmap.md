...
- ⏳ 首次启动依赖准备仍在 PowerShell 启动脚本里执行；已有粗粒度步骤进度，还没有安装器级进度页
...
- ⏳ 模型下载、Chromium 下载、依赖安装已有步骤级提示；下一步需要更细的下载进度和重试入口

## 📋 计划中（按优先级）

### 高优：个人工具可安装、可启动
1. **安装包首启体验**——依赖准备、Playwright 安装、数据库初始化已有步骤级进度，运行时下载缓存已收敛到 `D:\codexDownload\douyinclaude-runtime`，并已有独立重试入口和失败阶段提示；下一步继续做安装器级进度可视化
