...
> **历史文档，不代表当前实现。** 下方原文记录了 2026-05 当时选择“暂不迁移”的理由与清单，未按后续开发逐项打勾。当前事实请以 [README](../README.md)、[当前架构](architecture.md) 和 [当前路线图](roadmap.md) 为准。

## 当前状态（2026-07）

本地多账号与用户隔离基础已经落地：项目已有 users / invite / web sessions，本地账号可添加和切换；收藏与喜欢的核心主键使用 `(user_id, id)`，向量/全文索引、后台任务与相关日志按用户隔离；Playwright profile 使用 per-user 路径，CLI 也提供 `--user` 选择入口。

这不等于产品已经转向公网多用户或 SaaS。每用户 SMTP / `MAIL_TO`、正式权限模型与安全审计、计费配额和公共部署都没有完成，也不是近期方向。分类相关表仍保留 `account_id` 这一历史命名，不能把当前状态描述成租户字段已经完全统一。

---

## 历史原文（2026-05 决策快照，未逐项回填）
